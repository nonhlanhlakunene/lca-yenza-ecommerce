...
export const getReviewsByProfessionalController = async (req, res) => {
  try {
    ...
    if (!reviewedUserId) {
      return res.status(400).json({
        success: false,
        message: "reviewedUserId is required",
      });
    }

    const reviews = await getReviewsByProfessional(reviewedUserId);
